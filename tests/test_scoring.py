...
def seed_minimal(s):
    ...
    s.add(d)
    s.flush()
    th = ThemeORM(dimension_id=d.id, name="Oversight")
    s.add(th)
    s.flush()
    t1 = TopicORM(theme_id=th.id, name="Topic 1")
    t2 = TopicORM(theme_id=th.id, name="Topic 2")
    s.add_all([t1, t2])
    s.flush()
    for level, label in [
        (1, "Initial"),
...
def test_scoring_prefers_computed_score():
    ...
    with SessionLocal() as s:
        ...
        s.add(th)
        s.flush()
        t = TopicORM(theme_id=th.id, name="Topic")
        s.add(t)
        s.flush()
        for level, label in [
            (1, "Initial"),
